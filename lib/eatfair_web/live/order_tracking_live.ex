defmodule EatfairWeb.OrderTrackingLive do
      ...
      <div class="mt-8">
        <%= if @active_orders == [] do %>
          <div class="text-center py-12">
            ...
            <.link navigate={~p"/restaurants/discover"} class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <%= for order <- @active_orders do %>
              <div class="bg-white shadow-lg rounded-lg p-6 border border-gray-200">
                ...
                <div class="mt-6 flex justify-between items-center">
                  ...
                  <.link navigate={~p"/orders/track/#{order.id}"} class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    View Details →
                  </.link>
                </div>
              </div>
  ...
  defp render_order_status(order) do
    {icon, color, text} = case order.status do
  ...
  defp render_status_timeline(order) do
    ...
    current_index = Enum.find_index(statuses, fn {status, _, _} -> status == current_status end) || 0
    
    assigns = %{statuses: statuses, current_index: current_index, current_status: current_status, order: order}
    ...
    <div class="space-y-4">
      <%= for {{status, title, description}, index} <- Enum.with_index(@statuses) do %>
        ...
        <div class={["flex items-start", 
          ...
          <div class="ml-4">
            <h3 class={["font-semibold", is_current && "text-blue-900", is_completed && !is_current && "text-green-700"]}><%= title %></h3>
            <p class={["text-sm", is_current && "text-blue-700", is_completed && !is_current && "text-green-600", is_future && "text-gray-500"]}><%= description %></p>
  ...
  defp format_notification_message(event) do
    ...
    case data["new_status"] do
      "confirmed" -> "Your order from #{data["restaurant_name"]} has been confirmed!"
      "preparing" -> "Great news! #{data["restaurant_name"]} is now preparing your order."
      "ready" -> "Your order from #{data["restaurant_name"]} is ready for delivery!"
      "out_for_delivery" -> "Your order is on its way! Estimated arrival soon."
      "delivered" -> "Delivered! Enjoy your meal from #{data["restaurant_name"]}."
      "cancelled" -> "Unfortunately, your order from #{data["restaurant_name"]} has been cancelled."
