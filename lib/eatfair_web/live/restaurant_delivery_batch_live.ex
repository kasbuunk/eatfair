defmodule EatfairWeb.RestaurantDeliveryBatchLive do
    ...
    case Restaurants.get_restaurant_by_owner(current_user.id) do
      ...
      restaurant ->
        ...
        socket =
          ...
          |> assign(:batch_form, to_form(%{"name" => "", "scheduled_pickup_time" => "", "courier_id" => ""}))
  ...
  def handle_event("create_batch", batch_params, socket) do
    ...
    if Enum.empty?(selected_order_ids) do
      ...
      batch_attrs = %{
        ...
        courier_id: (if batch_params["courier_id"] != "", do: String.to_integer(batch_params["courier_id"]), else: nil),
      ...
      case Orders.create_delivery_batch(batch_attrs) do
        {:ok, batch} ->
          case Orders.assign_orders_to_batch(batch.id, selected_order_ids) do
            ...
            {:error, _reason} ->
              {:noreply, put_flash(socket, :error, "Failed to assign orders to batch. Please try again.")}
          end
  ...
  def render(assigns) do
          ...
          <div class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">
              <%= case @filter_status do %>
                <% :all -> %> No delivery batches created yet
                <% status -> %> No {status} batches
              <% end %>
            </h3>
                  ...
                  <div class="flex items-center justify-between">
                      ...
                      <div class="ml-4">
                        ...
                        <div class="text-sm text-gray-500">
                          ...
                          <%= if batch.scheduled_pickup_time do %>
                            <span class="ml-2">• Pickup: {format_datetime(batch.scheduled_pickup_time)}</span>
                          <% end %>
                        </div>
                    ...
                    <div class="flex items-center space-x-2">
                      <%= case batch.status do %>
                        ...
                        <% "proposed" -> %>
                          <span class="text-sm text-blue-600 font-medium">Awaiting courier response</span>
                          <.button 
                            phx-click="cancel_batch" 
                            phx-value-batch_id={batch.id}
                            class="bg-gray-600 hover:bg-gray-700 text-white text-sm px-3 py-1"
                        ...
                        <% "accepted" -> %>
                          <span class="text-sm text-green-600 font-medium">Accepted by courier</span>
                        <% "in_progress" -> %>
                    ...
                    <div class="mt-3 pl-12">
                      ...
                      <ul class="mt-1 text-sm text-gray-500">
                        ...
                        <%= if length(batch.orders) > 3 do %>
                          <li class="text-gray-400">...and {length(batch.orders) - 3} more orders</li>
                        <% end %>
                      </ul>
                    </div>
                  <% end %>
    ...
    <%= if @show_create_modal do %>
      <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" id="create-batch-modal">
        <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
          <div class="mt-3">
            ...
            <.form for={@batch_form} phx-submit="create_batch">
              ...
              <.input field={@batch_form[:scheduled_pickup_time]} type="datetime-local" label="Scheduled Pickup Time" />
              <.input 
              ...
              <div class="mt-6">
                ...
                <%= if Enum.empty?(@ready_orders) do %>
                  <div class="text-center py-8 bg-gray-50 rounded-lg">
                    ...
                    <p class="text-sm text-gray-400">Orders must be in "ready" status to be added to a batch</p>
                  </div>
                <% else %>
                        ...
                        <div class="flex-1">
                          <div class="flex justify-between items-start">
                            ...
                            <div class="text-right">
                              <p class="text-sm font-semibold text-gray-900">€{order.total_price}</p>
                              <p class="text-xs text-gray-500">{Enum.count(order.order_items)} items</p>
                            </div>
                          </div>
                        </div>
                      </label>
              ...
              <div class="mt-6 flex justify-end space-x-3">
                <.button type="button" phx-click="hide_create_modal" class="bg-gray-300 hover:bg-gray-400 text-gray-800">
                  Cancel
                </.button>
                <.button type="submit" class="bg-green-600 hover:bg-green-700 text-white" disabled={Enum.empty?(@ready_orders) || MapSet.size(@selected_orders) == 0}>
                  Create Batch
                </.button>
              </div>
            </.form>
