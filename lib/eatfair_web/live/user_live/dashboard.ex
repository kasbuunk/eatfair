defmodule EatfairWeb.UserLive.Dashboard do
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200" 
      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div class="stat bg-white dark:bg-gray-800 rounded-lg shadow p-6" role="region" aria-label="Total donations">
            <div class="stat-figure text-green-600">
              <.icon name="hero-heart" class="size-8" />
            </div>
            <div class="stat-title text-sm font-medium text-gray-500 dark:text-gray-400">Total Donations</div>
            <div class="stat-value text-2xl font-bold text-green-600">€<%= @donation_metrics.total_donations %></div>
            <div class="stat-desc text-green-500 text-sm">Supporting local restaurants</div>
          </div>

          <div class="stat bg-white dark:bg-gray-800 rounded-lg shadow p-6" role="region" aria-label="Restaurants supported">
            <div class="stat-figure text-blue-600">
              <.icon name="hero-building-storefront" class="size-8" />
            </div>
            <div class="stat-title text-sm font-medium text-gray-500 dark:text-gray-400">Restaurants Supported</div>
            <div class="stat-value text-2xl font-bold text-blue-600"><%= @impact_metrics.restaurants_supported %></div>
          ...
          <div class="stat bg-white dark:bg-gray-800 rounded-lg shadow p-6" role="region" aria-label="Reviews written">
            <div class="stat-figure text-purple-600">
              <.icon name="hero-star" class="size-8" />
            </div>
            <div class="stat-title text-sm font-medium text-gray-500 dark:text-gray-400">Reviews Written</div>
            <div class="stat-value text-2xl font-bold text-purple-600"><%= @impact_metrics.reviews_written %></div>
            <div class="stat-desc text-purple-500 text-sm">
              <%= pluralize(@impact_metrics.reviews_written, "review") %> written
            </div>
          </div>

          <div class="stat bg-white dark:bg-gray-800 rounded-lg shadow p-6" role="region" aria-label="Photos shared">
            <div class="stat-figure text-orange-600">
              <.icon name="hero-photo" class="size-8" />
            </div>
            <div class="stat-title text-sm font-medium text-gray-500 dark:text-gray-400">Photos Shared</div>
            <div class="stat-value text-2xl font-bold text-orange-600"><%= @impact_metrics.photos_shared %></div>
            ...
            <nav class="-mb-px flex space-x-8" aria-label="Dashboard sections">
              <button
                ...
                class={[
                  ...
                  (@section == :orders && "border-indigo-500 text-indigo-600") || "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                ]}
                tabindex="0"
                accesskey="1"
              >
              ...
              <button
                ...
                class={[
                  "py-2 px-1 border-b-2 font-medium text-sm transition-colors",
                  (@section == :reviews && "border-indigo-500 text-indigo-600") || "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                ]}
                tabindex="0"
                accesskey="2"
              >
              ...
              <button
                ...
                class={[
                  "py-2 px-1 border-b-2 font-medium text-sm transition-colors",
                  (@section == :impact && "border-indigo-500 text-indigo-600") || "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
                ]}
                tabindex="0"
                accesskey="3"
              >
                ...
                <div class="flex-1">
                  <div class="flex items-center space-x-4">
                    ...
                    <%= if order.donation_amount && Decimal.gt?(order.donation_amount, 0) do %>
                      <span data-test-id="donation-badge" 
                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        <.icon name="hero-heart" class="size-3 mr-1" />
                        €<%= :erlang.float_to_binary(Decimal.to_float(order.donation_amount), [{:decimals, 2}]) %> donated
                      </span>
                    <% end %>
                  </div>
                  ...
                  <div class="text-sm font-medium text-gray-900 dark:text-gray-100 mt-2">
                    Total: €<%= :erlang.float_to_binary(Decimal.to_float(order.total_price), [{:decimals, 2}]) %>
                  </div>
                </div>
                <span class={[
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
      ...
      <div class="p-6">
        <%= if Enum.any?(@reviews) do %>
          <div class="space-y-6">
            <div :for={review <- @reviews} class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center mt-1">
                    <div class="flex text-yellow-400">
                      <%= for _star <- 1..review.rating do %>★<% end %>
                      <%= if review.rating < 5 do %>
                        <%= for _star <- (review.rating + 1)..5 do %>☆<% end %>
                      <% end %>
                    </div>
      ...
      <div class="p-6">
        <div class="text-center">
          <%= if @donation_metrics.total_donations == "0.00" && @impact_metrics.restaurants_supported == 0 do %>
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Start supporting local restaurants</h3>
            <p class="text-gray-500 mb-6">Your contributions help maintain EatFair's zero-commission model</p>
            <.link navigate={~p"/restaurants"} 
          ...
          <% else %>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div class="text-center">
                <div class="text-3xl font-bold text-green-600 mb-2">€<%= @donation_metrics.total_donations %></div>
                <div class="text-gray-500">Total donated to support the platform</div>
              </div>
              
              <div class="text-center">
                <div class="text-3xl font-bold text-blue-600 mb-2"><%= @impact_metrics.restaurants_supported %></div>
            ...
            <div class="mt-8 p-4 bg-green-50 dark:bg-green-900 rounded-lg">
              <p class="text-green-800 dark:text-green-200 text-center">
                ...
                <%= if @impact_metrics.restaurants_supported == 1, do: "restaurant", else: "restaurants" %>
                with €<%= @donation_metrics.total_donations %> in donations
              </p>
            </div>
          <% end %>
        </div>
  ...
  def handle_event("edit_review", %{"id" => review_id}, socket) do
    {:noreply, push_patch(socket, to: ~p"/users/dashboard?action=edit_review&review_id=#{review_id}")}
  end
  ...
  defp load_impact_metrics(socket) do
    ...
    photos_shared = 
      from(ri in ReviewImage, 
        join: r in Eatfair.Reviews.Review, on: ri.review_id == r.id,
        where: r.user_id == ^user_id)
      |> Eatfair.Repo.aggregate(:count)
