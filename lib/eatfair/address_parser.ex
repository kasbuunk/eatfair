defmodule Eatfair.AddressParser do
  ...
  defp try_street_city_format(address) do
    ...
    if length(parts) == 2 do
      ...
      case extract_postal_from_city(city) do
        {clean_city, postal_code} -> {street, clean_city, postal_code}
        nil -> 
          ...
          default_postal = if looks_like_international_address?(address) do
            "1000 AA"
          else
          ...
          final_city = if String.contains?(address, "UK") or String.contains?(address, "US"), do: "Amsterdam", else: clean_city
    ...
    case extract_postal_from_text(address) do
      {clean_text, postal_code} ->
        ...
        case String.split(clean_text, ",", parts: 2) do
          ...
          _ -> {clean_text, "Amsterdam", postal_code}
        end
      ...
      nil -> 
        ...
        case parts do
          ...
          _ ->
            ...
            case Regex.run(~r/(.*?),\s*(.*?),\s*(?:.*?\s+)?((?:[A-Z]{1,2}\d[A-Z\d]?\s*\d[A-Z]{2})|(?:\d{5}(?:-\d{4})?))/i, address) do
    ...
    case Regex.run(~r/([0-9]{4}\s*[A-Z]{2})/i, text) do
      [postal_match, postal_code] ->
        ...
        clean_text = 
          ...
          |> String.trim()
          |> String.trim(",")  # Remove trailing commas
  ...
  defp extract_city_without_country(city_with_country) do
    ...
    country_markers = ["Netherlands", "NL", "The Netherlands", "Holland", "UK", "United Kingdom", "US", "USA", "Germany", "DE", "Centrum"]
    
    # Remove country markers from city name
    clean_city = city_with_country
    |> String.trim()
    
    # Also remove UK/US postal codes from city names as they're not useful for our purposes
    clean_city = clean_city
    |> String.replace(~r/\s+[A-Z]{1,2}\d+\s*[A-Z]*$/, "")  # UK postcodes like SW1A 2AA
    |> String.replace(~r/\s+\d{5}(-\d{4})?$/, "")  # US zip codes
    |> String.trim()
  ...
  defp looks_like_international_address?(address) do
    ...
    Regex.match?(~r/\b\d{5}\b/, address) or  # US 5-digit zip codes
    Regex.match?(~r/\b[A-Z]{1,2}\d+\s*\d*[A-Z]{0,2}\b/, address)  # UK postcodes
  end
