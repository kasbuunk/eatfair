defmodule Eatfair.Orders.Order do
  ...
  schema "orders" do
    ...
    belongs_to :courier, User  # For future courier assignment
  ...
  def changeset(order, attrs) do
    ...
    |> cast(attrs, [
      ...
      :customer_id, :restaurant_id, :courier_id,
      :confirmed_at, :preparing_at, :ready_at, :out_for_delivery_at, 
      :delivered_at, :cancelled_at, :estimated_delivery_at,
  ...
  def status_changeset(order, attrs) do
    order
    |> cast(attrs, [:status, :confirmed_at, :preparing_at, :ready_at, 
                    :out_for_delivery_at, :delivered_at, :cancelled_at,
                    :is_delayed, :delay_reason, :actual_prep_time_minutes])
    |> validate_inclusion(:status, @valid_statuses)
  ...
  defp validate_status_transition(changeset) do
    case {get_field(changeset, :status), changeset.data.status} do
      {new_status, old_status} when new_status == old_status -> changeset
      {"cancelled", _} -> changeset  # Can cancel from any status
      {"confirmed", "pending"} -> changeset
      {"preparing", "confirmed"} -> changeset
      {"ready", "preparing"} -> changeset
      {"out_for_delivery", "ready"} -> changeset
      {"delivered", "out_for_delivery"} -> changeset
