defmodule Eatfair.Restaurants.Restaurant do
  ...
  schema "restaurants" do
    ...
    field :contact_open_time, :integer, default: 540   # 09:00
    field :contact_close_time, :integer, default: 1320 # 22:00
    field :order_open_time, :integer, default: 600     # 10:00
    field :order_close_time, :integer, default: 1260   # 21:00
    field :kitchen_open_time, :integer, default: 600   # 10:00
    field :kitchen_close_time, :integer, default: 1320 # 22:00
    field :last_delivery_time, :integer, default: 1380 # 23:00
    field :order_cutoff_before_kitchen_close, :integer, default: 30
    field :min_prep_time_for_last_order, :integer, default: 15
    field :operating_days, :integer, default: 127      # All days
  ...
  defp validate_operational_hours(changeset) do
    ...
    |> validate_number(:order_cutoff_before_kitchen_close, greater_than_or_equal_to: 5, less_than_or_equal_to: 120)
    |> validate_number(:min_prep_time_for_last_order, greater_than_or_equal_to: 5, less_than_or_equal_to: 60)
  ...
  defp validate_time_range(changeset, open_field, close_field, description) do
    ...
    cond do
      ...
      open_time < 0 or open_time >= 1440 ->
        add_error(changeset, open_field, "#{description} open time must be between 00:00 and 23:59")
      
      close_time < 0 or close_time > 1440 ->
        add_error(changeset, close_field, "#{description} close time must be between 00:00 and 24:00")
      ...
      open_time == close_time ->
        add_error(changeset, close_field, "#{description} close time must be different from open time")
  ...
  defp validate_operational_logic(changeset) do
    _order_open = get_field(changeset, :order_open_time)
    ...
    cond do
      ...
      # Last order time should be before kitchen closes minus buffer
      order_close > (kitchen_close - cutoff_buffer) ->
        add_error(changeset, :order_close_time, "order acceptance must end #{cutoff_buffer} minutes before kitchen closes")
  ...
  defp valid_timezones do
    [
      ...
      "Europe/Rome", "Europe/Madrid", "Europe/Brussels", "Europe/Vienna",
  ...
  def open_for_orders?(%__MODULE__{} = restaurant) do
    if restaurant.force_closed do
      ...
      within_hours? = current_minute >= restaurant.order_open_time and 
  ...
  def last_order_time_today(%__MODULE__{} = restaurant) do
    if open_for_orders?(restaurant) do
      ...
      last_order_minute = restaurant.kitchen_close_time - restaurant.order_cutoff_before_kitchen_close
  ...
  def minutes_to_time(1440), do: "24:00"  # Handle end of day
      ...
      [hour_str, minute_str] ->
        with {hour, ""} <- Integer.parse(hour_str),
          ...
          if hour == 24 and minute == 0 do
            1440  # Special case for midnight next day
          else
            hour * 60 + minute
          end
        else
