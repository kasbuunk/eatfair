...
if night_owl_restaurant && test_customer && max_courier do
  IO.puts(
    "✅ Found Night Owl entities: Restaurant (#{night_owl_restaurant.id}), Customer (#{test_customer.id}), Courier (#{max_courier.id})"
  )
  ...
  if length(night_owl_meals) > 0 do
    ...
    build_night_owl_timestamps = fn status, minutes_ago ->
      ...
      case status do
        ...
        "confirmed" ->
          confirmed_time =
            DateTime.add(base_time, -5 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
        ...
        "preparing" ->
          confirmed_time =
            DateTime.add(base_time, -15 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
        ...
        "ready" ->
          confirmed_time =
            DateTime.add(base_time, -25 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)

          preparing_time =
            DateTime.add(base_time, -20 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
          ...
          %{
            ...
            ready_at: base_naive,
            courier_id: max_courier.id,
            courier_assigned_at: ready_time
          }

        "out_for_delivery" ->
          confirmed_time =
            DateTime.add(base_time, -35 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)

          preparing_time =
            DateTime.add(base_time, -30 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
          ...
          assigned_time =
            DateTime.add(base_time, -10 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
        ...
        "delivered" ->
          ...
          preparing_time =
            DateTime.add(base_time, -55 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
        ...
        "delivery_failed" ->
          confirmed_time =
            DateTime.add(base_time, -90 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)

          preparing_time =
            DateTime.add(base_time, -85 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)

          ready_time =
            DateTime.add(base_time, -65 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)

          out_time =
            DateTime.add(base_time, -45 * 60, :second)
            |> DateTime.to_naive()
            |> NaiveDateTime.truncate(:second)
    ...
    create_night_owl_order = fn order_attrs, order_spec ->
      ...
      total_price =
        if Decimal.compare(
             item_total,
             night_owl_restaurant.min_order_value || Decimal.new("12.00")
           ) == :lt do
          Decimal.add(
            night_owl_restaurant.min_order_value || Decimal.new("12.00"),
            Decimal.new("5.00")
    ...
    IO.puts(
      "🏗️ Creating #{length(night_owl_order_matrix)} core Night Owl orders (all statuses)..."
    )
    ...
    night_owl_core_orders =
      ...
      |> Enum.map(fn {order_spec, index} ->
        ...
        if rem(index, 3) == 0 or index <= 3,
          do: IO.puts("✅ Created #{order_spec.status} order ##{order.id}")
    ...
    night_owl_historical_orders =
      ...
      |> Enum.map(fn i ->
        ...
        # Night hours for Night Owl
        hour_offset = Enum.random(18..23)
        ...
        out_time =
          DateTime.add(historical_time, -20 * 60, :second)
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second)

        ready_time =
          DateTime.add(historical_time, -35 * 60, :second)
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second)

        prep_time =
          DateTime.add(historical_time, -50 * 60, :second)
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second)

        confirmed_time =
          DateTime.add(historical_time, -55 * 60, :second)
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second)

        assigned_time =
          DateTime.add(historical_time, -30 * 60, :second)
          |> DateTime.to_naive()
          |> NaiveDateTime.truncate(:second)
        ...
        order_attrs = %{
          ...
          donation_amount:
            if(rem(i, 4) == 0,
              do: Decimal.new("#{Enum.random(2..5)}.00"),
        ...
        order_spec = %{
          status: "delivered",
          notes: "Historical Night Owl order - #{days_ago} days ago"
        }
