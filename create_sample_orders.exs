          ...
          true ->
            ...
            orders_to_create = [
              ...
              %{
                ...
                address: "Amstel 1, Amsterdam"
              },
              %{
                status: "pending",
                customer: Enum.at(customers, 1),
                address: "Prinsengracht 100, Amsterdam"
              },
              %{
                status: "pending",
                customer: Enum.at(customers, 2),
                address: "Dam Square 5, Amsterdam"
              },

              # 2 confirmed orders (accepted, waiting to prepare)
              %{
                status: "confirmed",
                customer: Enum.at(customers, 3),
                address: "Vondelpark 10, Amsterdam"
              },
              %{
                status: "confirmed",
                customer: Enum.at(customers, 4),
                address: "Museumplein 20, Amsterdam"
              },

              # 2 preparing orders (kitchen is working)
              %{
                status: "preparing",
                customer: Enum.at(customers, 5),
                address: "Jordaan 50, Amsterdam"
              },
              %{
                status: "preparing",
                customer: Enum.at(customers, 6),
                address: "Nieuwmarkt 3, Amsterdam"
              },

              # 1 ready order (ready for pickup/delivery)
              %{
                status: "ready",
                customer: Enum.at(customers, 7),
              ...
              %{
                ...
                customer: Enum.at(customers, 0),
                address: "Rembrandtplein 8, Amsterdam"
              },
              ...
              %{
                status: "delivered",
                customer: Enum.at(customers, 1),
                address: "Centraal Station 1, Amsterdam"
              },
              %{
                status: "delivered",
                customer: Enum.at(customers, 2),
                address: "Arena 10, Amsterdam"
              },
            ...
            |> Enum.each(fn {order_data, index} ->
              ...
              selected_meals =
                all_meals |> Enum.take_random(min(Enum.random(1..3), length(all_meals)))
              ...
              total_price =
                if Decimal.compare(item_total, restaurant.min_order_value || Decimal.new("12.00")) ==
                     :lt do
              ...
              case Orders.create_order_with_items(order_attrs, items_attrs) do
                {:ok, order} ->
                  IO.puts(
                    "✅ Created #{order_data.status} order ##{order.id} for #{order_data.customer.name} (€#{total_price})"
                  )

                {:error, changeset} ->
                  IO.puts(
                    "❌ Failed to create #{order_data.status} order: #{inspect(changeset.errors)}"
                  )
              end
            end)
