defmodule Eatfair.OrderTestSupport do
  ...
  def transition_to_delivered(order) do
    ...
    with {:ok, preparing_order} <- Orders.update_order_status(confirmed_order, "preparing"),
         ...
         {:ok, out_for_delivery_order} <- Orders.update_order_status(ready_order, "out_for_delivery"),
  ...
  def transition_to_delivered!(order) do
    case transition_to_delivered(order) do
      {:ok, delivered_order} -> delivered_order
      {:error, changeset} -> raise "Failed to transition order to delivered: #{inspect(changeset.errors)}"
    end
  end
