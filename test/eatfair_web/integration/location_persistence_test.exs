defmodule EatfairWeb.LocationPersistenceTest do
  ...
  describe "Location persistence across restaurant discovery journey" do
    ...
    test "location parameter persists from discovery to restaurant detail page", %{conn: conn, restaurant: restaurant, customer: customer} do
      ...
      assert_redirect(discovery_view, ~p"/restaurants/#{restaurant.id}?location=#{search_location}")
      
      # 3. Follow the redirect to restaurant detail page  
      {:ok, restaurant_view, restaurant_html} = live(conn, ~p"/restaurants/#{restaurant.id}?location=#{search_location}")
      ...
      {:ok, _restaurant_view, logged_in_html} = live(conn, ~p"/restaurants/#{restaurant.id}?location=#{search_location}")
    ...
    test "direct navigation to restaurant page via link preserves location parameter", %{conn: conn, restaurant: restaurant} do
      search_location = "Utrecht"
    ...
    test "location persistence works with URL encoding for complex addresses", %{conn: conn, restaurant: restaurant} do
      ...
      assert_redirect(discovery_view, ~p"/restaurants/#{restaurant.id}?location=#{complex_location}")
    end

    test "fallback behavior when no location parameter is provided", %{conn: conn, restaurant: restaurant, customer: customer} do
    ...
    test "end-to-end flow: homepage -> discovery -> restaurant detail preserves location", %{conn: conn, restaurant: restaurant} do
      ...
      {:ok, discovery_view, discovery_html} = live(conn, ~p"/restaurants?location=#{search_address}")
      ...
      assert_redirect(discovery_view, ~p"/restaurants/#{restaurant.id}?location=#{search_address}")
      
      # 4. On restaurant detail page, delivery should be available
      {:ok, _restaurant_view, restaurant_html} = live(conn, ~p"/restaurants/#{restaurant.id}?location=#{search_address}")
      ...
      assert restaurant_html =~ "Your Order" or restaurant_html =~ "Cart" or restaurant_html =~ "Menu"
    end
  end
  ...
  describe "Location persistence edge cases" do
    test "handles invalid location parameters gracefully", %{conn: conn} do
      ...
      assert html =~ "No restaurants found" or html =~ "not found" or html =~ "adjust your filters"
    end
