  ...
  describe "🍽️ The Menu Creation Journey: From Concept to Customer" do
    test "Sarah builds her complete dumpling menu with categories and customization readiness" do
      ...
      dumpling_data = %{
        ...
        "description" =>
          "Crispy bottom, tender top. Made fresh daily with grandmother's secret seasoning blend. Served with our house-made soy-ginger dipping sauce.",
        "price" => "8.50"
      }
      ...
      assert has_element?(
               menu_live,
               "[data-test='item-available-toggle'][data-available='false']"
             )

      assert has_element?(
               menu_live,
               "[data-test='menu-item-pan-fried-pork-dumplings'][data-status='unavailable']"
             )
      ...
      main_dish_data = %{
        ...
        "description" =>
          "A generous selection of our best dumplings: pork, chicken, and vegetable. Perfect for sharing!",
        "price" => "24.00"
      }
      ...
      # At least appetizers and main courses
      assert length(restaurant_menus) >= 2

      all_meals = Restaurants.get_available_meals(restaurant.id)
      # At least the two items she created
      assert length(all_meals) >= 2
    ...
    test "restaurant owner can edit existing menu items seamlessly" do
      ...
      updated_data = %{
        ...
        "description" =>
          "Freshly baked sourdough with roasted garlic and herb butter. A customer favorite!",
        "price" => "6.00"
      }
