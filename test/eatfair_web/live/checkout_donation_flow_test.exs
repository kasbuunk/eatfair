defmodule EatfairWeb.CheckoutDonationFlowTest do
  ...
  setup do
    ...
    {:ok, menu} =
      Eatfair.Restaurants.create_menu(%{
        name: "Test Menu",
        restaurant_id: restaurant.id
      })

    {:ok, meal} =
      Eatfair.Restaurants.create_meal(%{
        name: "Test Pasta",
  ...
  describe "🎯 Donation Integration During Checkout" do
    test "customer can add optional donation and total updates in real-time", %{
      ...
      payment_url =
        ~p"/order/#{restaurant.id}/payment?cart=#{cart_encoded}&order_details=#{order_encoded}"
      ...
      # Default donation amount in input
      ...
      # Donation shows as +€3.00 in the UI
      assert html =~ "+€3.00"
      ...
      # Slightly longer than the 2s delay in payment processing
      Process.sleep(2100)
    ...
    test "donation amount is clearly shown in order confirmation", %{
      ...
      confirm_url =
        ~p"/order/#{restaurant.id}/confirm?cart=#{cart_encoded}&order_details=#{order_encoded}"
