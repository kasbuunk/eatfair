defmodule EatfairWeb.RestaurantLive.AnonymousCartFixTest do
  ...
  setup do
    ...
    meal1 =
      RestaurantsFixtures.meal_fixture(%{
  ...
  describe "anonymous user cart functionality" do
    test "anonymous user can view restaurant page without crashes - the original bug fix", %{
      conn: conn,
    ...
    test "anonymous user with location can potentially see cart buttons", %{
      conn: conn,
      restaurant: restaurant
    } do
    ...
    test "template handles nil current_scope gracefully throughout", %{
      conn: conn,
      restaurant: restaurant
    } do
      {:ok, _view, html} = live(conn, ~p"/restaurants/#{restaurant.id}")
    ...
    test "restaurant page loads with empty cart state for anonymous users", %{
      conn: conn,
      restaurant: restaurant
    } do
      {:ok, _view, html} = live(conn, ~p"/restaurants/#{restaurant.id}")
