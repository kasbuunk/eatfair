defmodule EatfairWeb.Live.Components.AddressAutocompleteInputChangeCrashTest do
  ...
  describe "CRITICAL BUG: input_change event crashes" do
    test "FIXED: input_change with _target parameter now handles gracefully" do
      ...
      socket = %Phoenix.LiveView.Socket{
        assigns: %{
          ...
          selected_index: -1,
          class: "block w-full rounded-lg border-0 bg-white px-4 py-4 text-gray-900 shadow-lg ring-1 ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-orange-500 text-lg",
      ...
      {:noreply, result_socket} = AddressAutocomplete.handle_event("input_change", crash_params, socket)
    ...
    test "handles normal input_change correctly" do
      ...
      {:noreply, result_socket} = AddressAutocomplete.handle_event("input_change", normal_params, socket)
  ...
  describe "after fix: defensive input_change handling" do
    test "handles malformed input_change parameters gracefully" do
      ...
      for params <- malformed_cases do
        {:noreply, result_socket} = AddressAutocomplete.handle_event("input_change", params, socket)
    ...
    test "extracts value from different parameter structures" do
      ...
      test_cases = [
        ...
        {%{"_target" => ["undefined"]}, ""},  # Should default to empty
