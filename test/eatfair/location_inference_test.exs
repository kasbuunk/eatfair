defmodule Eatfair.LocationInferenceTest do
  ...
  describe "infer_location/1" do
    ...
    test "returns medium confidence with session address when user has no saved addresses" do
      ...
      socket = %{
        assigns: %{current_scope: %{user: user}, session_location: "Utrecht, Netherlands"}
      }
    ...
    test "returns low confidence with IP-based geolocation when no location data available" do
      socket = %{assigns: %{}}
      ...
      assert result.address == "Amsterdam"
    end
