  ...
  describe "delivery batch management" do
    test "can create delivery batch with valid attributes" do
      ...
      batch_attrs = %{
        ...
        scheduled_pickup_time: NaiveDateTime.add(NaiveDateTime.utc_now(), 3600), # 1 hour from now
        estimated_delivery_time: NaiveDateTime.add(NaiveDateTime.utc_now(), 7200), # 2 hours from now
    ...
    test "validates delivery batch status transitions" do
      ...
      {:ok, batch} = Orders.create_delivery_batch(%{
        name: "Test Batch",
        restaurant_id: restaurant.id,
        courier_id: courier.id,
    ...
    test "can list delivery batches for restaurant" do
      ...
      {:ok, _batch1} = Orders.create_delivery_batch(%{
        name: "Morning Batch",
        restaurant_id: restaurant.id,
        courier_id: courier.id,
        scheduled_pickup_time: NaiveDateTime.add(NaiveDateTime.utc_now(), 3600),
      ...
      {:ok, _batch2} = Orders.create_delivery_batch(%{
        name: "Evening Batch", 
        restaurant_id: restaurant.id,
        courier_id: courier.id,
    ...
    test "can list delivery batches for courier" do
      ...
      {:ok, _batch} = Orders.create_delivery_batch(%{
        name: "Courier Batch",
        restaurant_id: restaurant.id,
        courier_id: courier.id,
        status: "accepted",
